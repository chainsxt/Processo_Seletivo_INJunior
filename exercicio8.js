...
function countBooksInCategory (booksByCategory) {
    ...
    for (let i=0; i<booksByCategory.length;i++){
        const categName = booksByCategory[i].category;
        const ttlBooks = booksByCategory[i].books.length;
         result[categName] = ttlBooks;

    }
    return result
}
const counts = countBooksInCategory(booksByCategory);
console.log(counts);
